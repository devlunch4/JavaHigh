...
public class DeadeokHotel {
	Map<Integer, Room_t> hotelMap;
	Scanner scan;

	// 생성자
	public DeadeokHotel() {
		hotelMap = new HashMap<Integer, Room_t>();
		...
		for(int i=2; i<=4; i++) {
			...
			for(int j=1; j<=9; j++) {
				int roomNum = i * 100 + j;
				Room_t room = new Room_t(roomNum, roomType);
	...
	private void showRoom() {
		...
		for(int i=0; i<roomNumList.size(); i++) {
			Room_t r = hotelMap.get( roomNumList.get(i)  );
...
class Room_t{
	...
	public Room_t(int roomNum, String roomType) {
		super();
